...
def create_vgg():
    ...
    for cfg in cfgs:
        if cfg == 'M': #floor #làm tròn xuống, cắt phần thập phân, lấy phần nguyên
            layers += [nn.MaxPool2d(kernel_size=2, stride=2)]
        elif cfg == 'MC': #ceiling # làm tròn lên
            layers += [nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True)]
            #tác giả SSD thì họ setting lại một số mode=True,
            #mục đích là để keep được thông tin của edge (cạnh) của feature map
            #Nếu để tất cả ceil_mode=False thì lúc lấy maxpooling sẽ bị mất thông tin của edge.
        else:
            conv2d = nn.Conv2d(in_channels, cfg, kernel_size=3, padding=1)

            layers += [conv2d, nn.ReLU(inplace=True)]# inplace = True sẽ quyết định lưu giá trị trên memory hay ko, true thì là ko lưu
            in_channels = cfg # set các layer tiếp theo, input sau = output trước ví dụ là ban đầu là 3 và chuyển sang 64
        
    pool5 = nn.MaxPool2d(kernel_size=3, stride=1, padding=1) # maxpooling thứ 5
    conv6 = nn.Conv2d(512, 1024, kernel_size=3, padding=6, dilation=6) #Nếu không có dilation thì các phần tử này xếp liền nhau và trượt trên ảnh như conv như thường, nhưng nếu có dilation thì các phần tử trong kernel sẽ cách nhau 1 khoảng bằng dilation và trượt trên ảnh.
...
def create_loc_conf(num_classes=21, bbox_aspect_num=[4, 6, 6, 6, 4, 4]):
    ...
    # source1
    # loc
    loc_layers += [nn.Conv2d(512, bbox_aspect_num[0]*4, kernel_size=3, padding=1)]#bbox_aspect_num[0]*4 nhân 4 vì 4 thông tin offset
    # conf
...
class SSD(nn.Module):
    def __init__(self, phase, cfg): # phase training or test or val
...
def nms(boxes, scores, overlap=0.45, top_k=200):
    """
    boxes: [num_box, 4] # num_box như là 8732 box, 4 ở đây là  xmin,ymin,xmax,ymax
    scores: [num_box]
    """
    count = 0
    # tạo tensor mới có dạng size(0) tức 1 cột nhiều hàng giống dtype của scores, 
    # và đặt cho các phần tử của hàng là 0, sau đó chuyển tensor về dạng long
    ...
    # Các thông số tính toán cho IoU
    tmp_x1 = boxes.new()
    tmp_x2 = boxes.new()
    tmp_y1 = boxes.new()
    tmp_y2 = boxes.new()
    ...
    value, idx = scores.sort(0) # sắp xếp các scores tăng dần từ hàng đầu đến hàng cuối
    idx = idx[-top_k:] # id của top 200 boxes có độ tự tin cao nhất

    while idx.numel() > 0: # nếu trong idx vẫn còn giá trị thì vẫn thực hiện tiếp
        i = idx[-1] # id của box có độ tự tin cao nhất
        keep[count] = i # giữ index giá trị lớn nhất
        count += 1

        if idx.size(0) == 1: # khi mà chỉ còn 1 bbox thì không còn gì để so sánh nữa thì kết thúc ctrinh
            break
        
        idx = idx[:-1] #lấy tất cả giá trị id của boxes ngoại trừ box có độ tự tin cao nhất
        #information boxes
        ...
        tmp_x1 = torch.clamp(tmp_x1, min=x1[i]) # =x1[i] if tmp_x1 < x1[i], nếu giá trị xmin của bbox nào đó nhỏ hơn xmin của bbox chính thì sẽ gán xmin của bbox đó giống vs xmin của bbox chính
        ...
        tmp_w = torch.clamp(tmp_w, min=0.0) # nếu tmp_w có phần tử âm thì đưa về 0
        tmp_h = torch.clamp(tmp_h, min=0.0)

        # overlap area
        inter = tmp_w*tmp_h #phần diện tích trùng nhau
        others_area = torch.index_select(area, 0, idx) # diện tích của mỗi bbox
        union = area[i] + others_area - inter # area[i] là diện tích phần bbox chính
        iou = inter/union
        idx = idx[iou.le(overlap)] # giữ lại id của box có overlap ít hơn với bbox đang xét
